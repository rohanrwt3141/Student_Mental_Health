...
st.set_page_config(page_title="Student Mental Health Portal", layout="wide")
...
def login_user(username, password):
    df = pd.read_csv(USER_DB)
    hashed_p = make_hashes(password)
    user_record = df[(df['username'] == username) & (df['password'] == hashed_p)]
    return not user_record.empty


# --- 3. THE MAIN PREDICTOR CODE ---
def main_predictor_page():
    ...
    st.info("Welcome to the main analysis dashboard.")

    st.markdown("""<h1 style = "color: #FA5C5C;"><strong>Mental Stress Prediction</strong></h1>""",
    ...
    X = df[["exam_encoder", "assignment_encoder", "academic_encoder", "sleep_encoder", "physical_encoder",
            "screen_encoder", "eating_encoder", "family_encoder", "friends_encoder", "living_encoder"]]
    y = df["mental_encoder"]

        # print(df.nunique())
        # print(df["Academic_Performance"].unique())
    ...
    train_acc = accuracy_score(y_train, train_pred)
        ...
        # Testing accuracy (already present)
    test_acc = accuracy_score(y_test, y_pred)

        # print(f"Training Accuracy: {train_acc}")
        # print(f"Testing Accuracy: {test_acc}")
        # print(f"Classification Report Train : {classification_report(y_train, train_pred)}")
        # print(f"Classification Report Test: {classification_report(y_test, train_pred)}")

        # DATA FROM THE USER IS TAKEN
    st.set_page_config(page_title="Mental Health App", layout="wide")

    st.markdown(""" <h5 style = "color: #FD8A6B; "> <strong> Enter The Exam Stress : </strong> </h5> """,
                    unsafe_allow_html=True)
    exam = st.selectbox("", list(le_exam.classes_))

    st.markdown(""" <h5 style = "color: #FD8A6B; "> <strong> Enter The Assignment Workload : </strong> </h5> """,
                    unsafe_allow_html=True)
    assignment = st.selectbox("", list(le_assignment.classes_))

    st.markdown(""" <h5 style = "color: #FD8A6B; "> <strong> Enter The Academic Performance : </strong> </h5> """,
                    unsafe_allow_html=True)
    academic = st.selectbox("", list(le_academic.classes_))

    st.markdown(""" <h5 style = "color: #FD8A6B; "> <strong> Enter The Sleep Duration : </strong> </h5> """,
                    unsafe_allow_html=True)
    sleep = st.selectbox("", list(le_sleep.classes_))

    st.markdown(
            """ <h5 style = "color: #FD8A6B; "> <strong> Enter The Physical Activity Level : </strong> </h5> """,
            unsafe_allow_html=True)
    physical = st.selectbox("", list(le_physical.classes_))

    st.markdown(""" <h5 style = "color: #FD8A6B; "> <strong> Enter The Screen Time : </strong> </h5> """,
                    unsafe_allow_html=True)
    screen = st.selectbox(f"", list(le_screen.classes_))

    st.markdown("""<h5 style = "color: #FD8A6B; "> <strong> Enter Your Eating Habit : </strong> </h5> """,
                    unsafe_allow_html=True)
    eating = st.selectbox(f"", list(le_eating.classes_))

    st.markdown("""<h5 style = "color: #FD8A6B; "> <strong> Enter Your Family Support Level : </strong> </h5> """,
                    unsafe_allow_html=True)
    family = st.selectbox(f"", list(le_family.classes_), key="family")

    st.markdown("""<h5 style = "color: #FD8A6B; "> <strong> Enter Friend Support Level : </strong> </h5> """,
                    unsafe_allow_html=True)
    friend = st.selectbox(f"", list(le_friend.classes_))

    st.markdown("""<h5 style = "color: #FD8A6B; "> <strong> Enter The Living Condition : </strong> </h5> """,
                    unsafe_allow_html=True)
    ...
    T_data = {"exam_encoder": T_exam, "assignment_encoder": T_assignment, "academic_encoder": T_academic,
                  "sleep_encoder": T_sleep, "physical_encoder": T_physical, "screen_encoder": T_screen,
                  "eating_encoder": T_eating, "family_encoder": T_family, "friends_encoder": T_friend,
                  "living_encoder": T_living}
    ...
    if (st.button("Predict")):
        if (prediction == 2):
            st.markdown(""" <h5 style = "color: yellow; "> <strong> <em> Moderate </em> </strong> </h5> """,
                            unsafe_allow_html=True)

        elif (prediction == 0):
            st.markdown(""" <h5 style = "color: green; "> <strong> <em> Heathy </em> </strong> </h5> """,
                            unsafe_allow_html=True)

        elif (prediction == 1):
            st.markdown(""" <h5 style = "color: red; "> <strong> <em> High Stress </em> </strong> </h5> """,
                            unsafe_allow_html=True)
        else:
            st.write("Error")
        ...
        if (st.button("Solution")):
            ...
            # Sleep Cycle
            st.markdown(""" <h4 style = "color: black; background: #ccc; "> About Your Sleep Cycle  </h4> """,
                        unsafe_allow_html=True)

        if (sleep == "More than 8 hours" or sleep == "7-8 hours"):
            st.markdown(
                    """ <h5 style = "color: green; background: #ccc;"> Your Sleep Cycle is Very Good..... Keep It Up </h5>""",
                    unsafe_allow_html=True)
        else:
            st.markdown(
                    """<h4 style = "color: blue; background: #ccc;" > 7 to 8 hours sleep is required for a Student. </h4>""",
                    unsafe_allow_html=True)
            st.markdown(
            ...
            st.markdown(
                    """<a href = "https://www.youtube.com/results?search_query=Improve+Sleep+Cycle+" target = "_blank" ><button style=\'padding:10px 20px; font-size:16px;'> More Videos </button></a>""",
                    unsafe_allow_html=True)
        ...
        st.markdown(
            ...
            # Exam Stress
        st.markdown(""" <h4 style = "color: black; background: #ccc; "> About Your Exam Stress  </h4> """,
                        unsafe_allow_html=True)

        if (exam == "Low"):
            st.markdown(
                    """ <h5 style = "color: green; background: #ccc"> Your Exam Stress is Very Good..... Keep It Up </h5>""",
                    unsafe_allow_html=True)
        elif (exam == "Very Low"):
            st.markdown(""" <p style = "color: black; background: #ccc; "> Your Exam Stress is Very Low  </p> """,
                            unsafe_allow_html=True)
            st.markdown(
                    """ <p style = "color: black; background: #ccc; "> Experiencing very low exam stress 😊 may feel comfortable and relaxed, but it can sometimes create hidden challenges. When stress is extremely low, a student may become overconfident 😌 and assume that preparation is already sufficient. This overconfidence can lead to procrastination ⏳, where study tasks are delayed because there is no sense of urgency. Gradually, the seriousness toward the exam may decrease 📉, and important topics might not receive proper attention. As the exam date approaches, this relaxed attitude can suddenly turn into last-minute pressure 😰, forcing rushed revision and unnecessary stress. </p>""",
                    unsafe_allow_html=True)

        else:
            st.markdown(
                    """ <h5 style = "color: blue; background: #ccc;" > Low Exam Stress is required for a Student </h5> """,
                    unsafe_allow_html=True)
            st.markdown(
                    """ <h5 style = "color: black; background: #ccc;" > I suggest you to watch this video and apply in your life </h5> """,
                    unsafe_allow_html=True)
            ...
            st.markdown(
                    """ <a href = "https://www.youtube.com/results?search_query=Serious+video+for+Exam+if+we+have+very+low+stress" target = "_blank" ><button style="padding:10px 20px; font-size:16px;"> More Videos </button> </a> """,
                    unsafe_allow_html=True)
        ...
        st.markdown(
            ...
            # Assignment Workload
        st.markdown(
                """ <pre> <h4 style = "color: black; background: #ccc; ">                About Your Assignment Workload  </h4> </pre>""",
                unsafe_allow_html=True)
        if (assignment == "Moderate"):
            st.markdown(
                    """ <p style = "color: black; background: #ccc; "> Your Assignment Workload is Moderate.  </p> """,
                    unsafe_allow_html=True)
            ...
            st.markdown(
                    """ <a href = "https://www.youtube.com/results?search_query=how+to+balance+assignment+workload+of+college" target = "_blank" ><button style="padding:10px 20px; font-size:16px;"> More Videos </button> </a> """,
                    unsafe_allow_html=True)
            ...
            if (physical == "Daily" or physical == "Regularly"):
                st.markdown(
                        """ <p style = "color: black; background: #ccc; "> Your Physical Activity is Good for Healthy Stress.  </p> """,
                        unsafe_allow_html=True)
                ...
                st.markdown(
                    ...
                    # Screen Time
                st.markdown(
                        """ <pre> <h4 style = "color: black; background: #ccc; ">                About Your Screen Time  </h4> </pre>""",
                        unsafe_allow_html=True)
                if (screen == "2-4 hours" or screen == "Less than 2 hours"):
                    st.markdown(
                            """ <p style = "color: black; background: #ccc; "> Your Screen Time is Good for the Productive Use .  </p> """,
                            unsafe_allow_html=True)

                else:
                    st.markdown(
                            """ <h5 style = "color: blue; background: #ccc;" > Screen Time of less than 4 hours only for productive Use is required for a Student </h5> """,
                            unsafe_allow_html=True)
                    st.markdown(
                            """ <h5 style = "color: black; background: #ccc;" > I suggest you to watch this video and apply in your life </h5> """,
                            unsafe_allow_html=True)
                    ...
                    st.markdown(
                            """ <a href = "https://www.youtube.com/results?search_query=Screen+Time+Problem" target = "_blank" ><button style="padding:10px 20px; font-size:16px;"> More Videos </button> </a> """,
                            unsafe_allow_html=True)
